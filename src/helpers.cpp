...
static HRESULT _ProtectAndCopyString(
    ...
    *ppwzProtected = nullptr;
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (!CredProtectW(FALSE, pwzToProtectCopy, (DWORD) wcslen(pwzToProtectCopy) + 1, nullptr, &cchProtected,
                          nullptr)) {
            ...
            if ((ERROR_INSUFFICIENT_BUFFER == dwErr) && (0 < cchProtected)) {
                ...
                if (pwzProtected) {
                    ...
                    if (CredProtectW(FALSE, pwzToProtectCopy, (DWORD) wcslen(pwzToProtectCopy) + 1, pwzProtected,
...
HRESULT KerbInteractiveUnlockLogonRepackNative(
    ...
    *pcbNative = 0;
    ...
    CredUnPackAuthenticationBufferW(CRED_PACK_WOW_BUFFER, rgbWow, cbWow, pszDomainUsername, &cchDomainUsername, nullptr,
                                    nullptr, pszPassword, &cchPassword);
...
HRESULT SplitDomainAndUsername(_In_ PCWSTR pszQualifiedUserName, _Outptr_result_nullonfailure_ PWSTR *ppszDomain,
    ...
    *ppszUsername = nullptr;
    ...
    if (pchWhack != nullptr) {
        ...
        // number of actual chars, NOT INCLUDING null terminated string
